...
> - **Extraction accuracy.** Field accuracy, exact-match and abstention
>   precision/recall need hand labels (`make label`), and those are the one
>   input in this project that cannot be generated by the project. Grading the
>   LLM against labels an LLM produced would measure agreement, not accuracy.
>   Self-consistency is *not* in this bucket - it needs no labels and is
>   reported, which is exactly why it is worth reporting: an unstable field is
>   unusable downstream whatever a small gold sample says about it.
>
...
tests/                 66 tests; test_leakage.py covers the split/timing logic
```
